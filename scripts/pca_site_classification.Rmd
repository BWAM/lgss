...
fielddata.pca.data <- fielddata.num.sub.hab %>% 
...
fielddata.pca.habitatonly <- fielddata.pca.data %>%
  select(unique_id, epifaunal_cover, velocity_depth_regime, riffle_bend_frequency, left_bank_stability, right_bank_stability, left_bank_veg, right_bank_veg, left_bank_veg_zone, right_bank_veg_zone)

# Remove habitat variables and retain just total habitat score
fielddata.pca.data.clean <- fielddata.pca.data %>% 
  select(-c(epifaunal_cover, velocity_depth_regime, riffle_bend_frequency, left_bank_stability, right_bank_stability, left_bank_veg, right_bank_veg, left_bank_veg_zone, right_bank_veg_zone))

# Subset to only variables that represent disturbance
fielddata.pca.disturb <- fielddata.pca.data.clean %>% 
  select(unique_id, landcover_nat_pct, landcover_dev_pct, landcover_ag_pct, landcover_water_pct, spcond, ph, habitat_total_score)

# Subset to only variables that represent disturbance, minus duplicate landcover and pH
fielddata.pca.disturb.simp.nat <- fielddata.pca.data.clean %>% 
  select(unique_id, landcover_nat_pct, spcond, habitat_total_score)

# Same as above using DEV instead of NAT landcover
fielddata.pca.disturb.simp.dev <- fielddata.pca.data.clean %>% 
  select(unique_id, landcover_dev_pct, spcond, habitat_total_score)
