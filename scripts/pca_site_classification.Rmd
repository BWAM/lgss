...
class.plot.df <- scores %>% 
  ...
  # Force 4 sites to degraded based on BPJ.
  mutate(basin = substr(site_id, 0, 2),
         PC1 = case_when(
           site_id %in% "11-TOMH-12.2" ~ -5,
           site_id %in% "09-LSUK-11.3" ~ -5,
           site_id %in% "16-WEBA-23.3" ~ -5,
           site_id %in% "13-IDNK-0.5" ~ -5,
           TRUE ~ PC1
         ))
...
all.abiotic.classified<-left_join(fielddata.lc, class.plot.df, by=c("site_id")) %>%
  ...
  mutate(condition_class = case_when(
    PC1 <= -0.93280967 ~ "3", # Highly Disturbed
    PC1 <= -0.06879499 ~ "2", # Moderately Disturbed (Q3)
    PC1 <= 0.98344902 ~ "2", # Slightly Disturbed (Q2) (grouped with above becuase we're only testing 1 and 3)
    PC1 > 0.98344902 ~ "1", # Least Disturbed
    TRUE                 ~"NOT CLASSIFIED"
  )) %>%
...
class.plot <- class.plot.df %>% 
  ...
  geom_vline(xintercept = c(-0.93280967,-0.06879499,0.98344902), linetype = "dotdash") +
...
write.table(all.abiotic.classified, file.path(root.dir, "data/compile_2015-2019/field_data/2015-2019_LG_field_landcover_condclasses_20210608-reclass.csv"),sep=",", row.names = FALSE)

# Export just the classes for joining elsewhere
classes.export <- all.abiotic.classified %>%
  select(site_id, condition_class, PC1)
write.table(classes.export, file.path(root.dir, "data/compile_2015-2019/condition_classes/2015-2019_LG_condclasses_20210608-reclass.csv"),sep=",", row.names = FALSE)
