...
# Same as initial but ran data with adjusted Long Island pH values
# > quantile(class.plot.df$PC1, probs = seq(0, 1, 0.25), na.rm = FALSE, names = TRUE, type = 7)
#         0%        25%        50%        75%       100% 
# -5.2356121 -0.9447973 -0.1306958  1.1430231  4.2289229 
...
#   select(-basin.y) %>% 
#   mutate(condition_class = case_when(PC1 <= -0.8870725 ~ "3", # Highly Disturbed
#                                      PC1 <= 0.0270505 ~ "2", # Moderately Disturbed (Q3)
#                                      PC1 <= 1.1074533 ~ "2", # Slightly Disturbed (Q2) (grouped with above becuase we're only testing 1 and 3)
#                                      PC1 > 1.1074533 ~ "1", # Least Disturbed
#                                      TRUE                 ~"NOT CLASSIFIED"
#   )) %>% 
#   select(site_id, condition_class, everything(), -PC2)

## Same as initial but w/ 17-CARL-0.8 omitted (bad field data)
##           0%          25%          50%          75%         100%
## -5.277933445 -0.889305603 -0.008391001  1.127265263  3.620548435
# all.abiotic.classified<-left_join(fielddata.lc, class.plot.df, by=c("site_id")) %>% 
#   filter(!(site_id %in% c("03-LSAM_N-7.9", "01-BLKE-0.1", "17-CARL-0.8"))) %>% 
#   rename(basin = basin.x) %>% 
#   select(-basin.y) %>% 
#   mutate(condition_class = case_when(PC1 <= -0.889305603 ~ "3", # Highly Disturbed
#                                      PC1 <= -0.008391001 ~ "2", # Moderately Disturbed (Q3)
#                                      PC1 <= 1.127265263 ~ "2", # Slightly Disturbed (Q2) (grouped with above becuase we're only testing 1 and 3)
#                                      PC1 > 1.127265263 ~ "1", # Least Disturbed
#                                      TRUE                 ~"NOT CLASSIFIED"
#   )) %>% 
#   select(site_id, condition_class, everything(), -PC2)
...
all.abiotic.classified<-left_join(fielddata.lc, class.plot.df, by=c("site_id")) %>%
  ...
  rename(basin = basin.x) %>%
  select(-basin.y) %>%
  mutate(condition_class = case_when(PC1 <= -0.88806864 ~ "3", # Highly Disturbed
                                     PC1 <= -0.01500482 ~ "2", # Moderately Disturbed (Q3)
                                     PC1 <= 1.12320423 ~ "2", # Slightly Disturbed (Q2) (grouped with above becuase we're only testing 1 and 3)
                                     PC1 > 1.12320423 ~ "1", # Least Disturbed
                                     TRUE                 ~"NOT CLASSIFIED"
  )) %>%
  select(site_id, condition_class, everything(), -PC2)
...
class.plot <- class.plot.df %>% 
  ...
  ### Incorrect intercepts commented out below... WHERE DID THESE COME FROM? ###
  # geom_vline(xintercept = c(-1.4769168,-0.4725721,0.9480725))+
  # geom_vline(xintercept = c(-0.8870725,1.1074533))+
  # geom_vline(xintercept = 0.0270505, linetype="dotdash")+
...
write.table(all.abiotic.classified, file.path(root.dir, "data/compile_2015-2019/field_data/2015-2019_LG_field_landcover_condclasses_20210526_CARL-pHfixed.csv"),sep=",", row.names = FALSE)

# Export just the classes for joining elsewhere
classes.export <- all.abiotic.classified %>%
  select(site_id, condition_class, PC1)
write.table(classes.export, file.path(root.dir, "data/compile_2015-2019/condition_classes/2015-2019_LG_condclasses_20210526_CARL-pHfixed.csv"),sep=",", row.names = FALSE)
