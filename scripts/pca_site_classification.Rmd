...
pca_biplot +
  geom_vline(xintercept = c(-0.87304171, 0.98497815), linetype = "dotdash") +
...
all.abiotic.classified<-left_join(fielddata.lc, class.plot.df, by=c("site_id")) %>%
  ...
  mutate(condition_class = case_when(
    ...
    PC1 <= 0.98497815 ~ "2", # Slightly Disturbed (Q2) (grouped with above becuase we're only testing 1 and 3)
    PC1 > 0.98497815 ~ "1", # Least Disturbed
    TRUE                 ~"NOT CLASSIFIED"
  )) %>%
...
class.plot <- class.plot.df %>% 
  ...
  geom_vline(xintercept = c(-0.87304171,-0.04705769,0.98497815), linetype = "dotdash") +
