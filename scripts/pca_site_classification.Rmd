...
quantile(scores_filter$PC1, probs = seq(0, 1, 0.25), na.rm = FALSE, names = TRUE, type = 7)

# for fielddata.pca.disturb2:
#         0%        25%        50%        75%       100% 
# -5.4314219 -0.8870725  0.0270505  1.1074533  3.2054342 
        
all.abiotic.classified<-left_join(fielddata.lc, scores_filter, by=c("site_id")) %>% 
  filter(!(site_id %in% c("03-LSAM_N-7.9", "01-BLKE-0.1"))) %>% 
  mutate(condition_class = case_when(PC1 <= -0.8870725 ~ "HD", # Highly Disturbed
                                     PC1 <= 0.0270505 ~ "MD", # Moderately Disturbed
                                     PC1 <= 1.1074533 ~ "SD", # Slightly Disturbed
                                     PC1 > 1.1074533 ~ "LD", # Least Disturbed
                                     TRUE                 ~"NOT CLASSIFIED"
  )) %>% 
  select(site_id, condition_class, everything(), -PC1)
...
p <- all.abiotic.classified.long %>% 
  mutate(condition_class = factor(condition_class, c("LD", "SD", "MD", "HD"))) %>%  
...
class.plot <- class.plot.df %>% 
  ...
  annotate("text", x = -4, y = -4, label = "Highly\nDisturbed")+
  annotate("text", x = -0.9, y = -4, label = "Moderately\nDisturbed")+
  annotate("text", x =  0.3, y = -4, label = "Slightly\nDisturbed")+
  annotate("text", x =  2.5, y = -4, label = "Least\nDisturbed")
class.plot

# List unique basins in each category
class.HD.basins <- all.abiotic.classified %>% 
  select(site_id, basin, condition_class) %>% 
  filter(condition_class == "HD")
class.MD.basins <- all.abiotic.classified %>% 
  select(site_id, basin, condition_class) %>% 
  filter(condition_class == "MD")
class.SD.basins <- all.abiotic.classified %>% 
  select(site_id, basin, condition_class) %>% 
  filter(condition_class == "SD")
class.LD.basins <- all.abiotic.classified %>% 
  select(site_id, basin, condition_class) %>% 
  filter(condition_class == "LD")
