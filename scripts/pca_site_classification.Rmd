...
fielddata.path <- file.path(root.dir, "data", "compile_2015-2019", "field_data", "final_cleaned", "2015-2019_lowgrad_fielddata_20201117.csv")
landcover.path <- file.path(root.dir, "data", "compile_2015-2019", "landcover", "landcover_final_2015-2019_20201118.csv")
...
fielddata <- read.csv(fielddata.path, stringsAsFactors = FALSE) %>% 
  ...
  select(unique_id, everything(), -event_id) %>% 
  arrange(site_id, sample_date) %>% 
  distinct(site_id, .keep_all = TRUE)

# Import landcover data generated in landcover.Rmd and clean up site IDs to match
...
fielddata.num <- fielddata %>% 
  select(unique_id, which(sapply(.,is.numeric)), -c(basin))
...
# Filter out spotty params
  select(-c(hab_present, embeddedness, chl_a, microtox, biosample, habitat_total_score, clay, rubble, silt, rock, sand, gravel, salinity, algae_diaoms_presen, thickness_diatoms_present, cond_test_250, land_use_pct_natural_wetland, lgs_habitat_snag, lgs_habitat_bank, lgs_habitat_macrophyte, lgs_habitat_substrate)) %>% 
...
# Filter out sites with lots of missing data
  filter(!(unique_id %in% c("03-LSAM_N-7.9_2015", "01-BLKE-0.1_2015"))) %>% 
...
# Fill in dummy values and calculate total habitat score
fielddata.num.sub.hab <- fielddata.num.sub %>% 
  
  ###
  ### fill in dummy vals here with means of given column? ###
  ###
...
datacounts2 <- data.frame(colname = names(fielddata.num.sub.hab),colSums_fielddata=colSums(!is.na(fielddata.num.sub.hab)))

# Remove incomplete fields for testing PCA
fielddata.pca.test <- fielddata.num.sub.hab %>% 
...
fielddata.pca.test2<-fielddata.pca.test %>% 
  column_to_rownames(var = "unique_id")

# PCA setup and biplot
    ## Scale and center data 
fielddata.pca.test3<-scale(fielddata.pca.test2, center = TRUE, scale = TRUE)
abiotic.pca <- prcomp(fielddata.pca.test3, cor = TRUE)

# Plot PCA Biplot and quantiles of PC1 values
pca_biplot<-autoplot(abiotic.pca, data = fielddata.pca.test3, label = F, label.size = 3, loadings.label = TRUE, loadings.label.repel=T, loadings.label.size = 4, scale= 0)
