...
# write.csv(sens.filt, file.path(root.dir, "data/metrics/sens.filt_seed80new_20210113.csv"))
```
...
# Only retain unique sites used in IBI dev (some were resampled in 2017-2019)
  ...
  select(-c(sample_date, uniqueid, "temperature of ph analysis", "conductivity at 25 degrees celsius", "ph", "chlorophyll a", "silver"
...
metric.chem.corscore2 <- metric.param.corr.chem %>% 
  ...
  mutate(corrscore = abs(rowSums(.[2:20]))) %>% 
...
metrics.scores<-metrics.scores %>%
  group_by(site_id) %>%
  mutate(SITE_SCORE = sum(c(dom_5_ttaxa, pct_gatherer, rich_ttaxa, rich_ttaxa_toe, rich_ttaxa_intol_facul, shannon_ttaxa_toe, pct_rich_ttaxa_cte))) %>%
  mutate(SITE_SCORE = SITE_SCORE/7) %>%
  ungroup() %>%
  select(site_id, SITE_SCORE, dom_5_ttaxa, pct_gatherer, rich_ttaxa, rich_ttaxa_toe, rich_ttaxa_intol_facul, shannon_ttaxa_toe, pct_rich_ttaxa_cte)
