...
metrics.scores<-metrics.scores %>%
  group_by(site_id) %>%
  mutate(SITE_SCORE = sum(c(dom_5_ttaxa, pct_gatherer, rich_ttaxa, pct_rich_ttaxa_toe, rich_ttaxa_tol_0to7, shannon_ttaxa_toe))) %>%
  # mutate(SITE_SCORE = sum(c(dom_5_ttaxa, pct_gatherer, rich_ttaxa, pct_rich_ttaxa_toe, shannon_ttaxa_toe))) %>%
  mutate(SITE_SCORE = SITE_SCORE/7) %>%
  ungroup() %>%
  select(site_id, SITE_SCORE, dom_5_ttaxa, pct_gatherer, rich_ttaxa, pct_rich_ttaxa_toe, rich_ttaxa_tol_0to7, shannon_ttaxa_toe)
  # select(site_id, SITE_SCORE, dom_5_ttaxa, pct_gatherer, rich_ttaxa, pct_rich_ttaxa_toe, shannon_ttaxa_toe)
...
#       0%      25%      50%      75%     100% 
# 41.57398 48.78793 62.55053 70.24540 80.41594 
...
quantile(metrics.scores.c3train$SITE_SCORE, probs = c(0, 0.25, 0.5, 0.75, 1))
 #       0%       25%       50%       75%      100% 
 # 4.224555 17.464382 27.705338 38.694969 55.958934 
...
metrics.scores.rescale <- metrics.scores %>% 
  mutate(SITE_SCORE_FINAL = (((SITE_SCORE - 17.464382)/(70.24540-17.464382)) * 10)) %>% 
...
write_csv(metrics.scores.rescale, file.path(root.dir, "data/metrics/final_scores/metric_scores_all_lgbap_scaled_20210222.csv"))
