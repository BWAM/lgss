...
# write.csv(sens.filt, file.path(root.dir, "data/metrics/june2021_rerun/sens.filt_seed70_2021-06-11b.csv"))
```
...
metrics.scores<-metrics.scores %>%
  group_by(site_id) %>%
  mutate(SITE_SCORE = sum(c(rich_ttaxa_tol_0to7, pct_rich_ttaxa_coe, shannon_ttaxa_shredder, dom_4_ttaxa, shannon_ttaxa_pote))) %>%
  mutate(SITE_SCORE = SITE_SCORE/5) %>%
  ungroup() %>%
  select(site_id, SITE_SCORE, rich_ttaxa_tol_0to7, pct_rich_ttaxa_coe, shannon_ttaxa_shredder, dom_4_ttaxa, shannon_ttaxa_pote)
...
#       0%      25%      50%      75%     100% 
# 22.53083 64.23811 76.35478 85.97268 98.35651 
...
quantile(metrics.scores.c3train$SITE_SCORE, probs = c(0, 0.25, 0.5, 0.75, 1))
 #      0%      25%      50%      75%     100% 
# 16.81730 21.40051 31.93981 45.01168 85.62857 
...
metrics.scores.rescale <- metrics.scores %>% 
  mutate(SITE_SCORE_FINAL = (((SITE_SCORE - 21.40051)/(85.97268 - 21.40051)) * 10)) %>% 
