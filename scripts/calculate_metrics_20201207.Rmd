...
#### Alternative method using midpoint to standardize
## For metrics decreasing w disturbance:
##   midpoint = abs(25th of LD - 75th of MD)
##   ceiling =  75th of LD
##   floor = midpoint - (75th of LD - midpoint)
## For metrics increasing w disturbance:
##   midpoint = abs(75th of LD - 25th of MD)
...
metrics.thresholds <- metrics.df.standardized.long %>%
  ...
  mutate(midpoint_incr = colMeans(metrics.thresholds[ , c(q25_1, q75_3)]))
    
  #   midpoint = case_when(
  #   disturbance == "decrease" ~ mean(c(q25_1, q75_3)),
  #   disturbance == "increase" ~ mean(c(q75_1, q25_3))
  #   )
  # )
...
# Determine 25th of class 1 and 75th of class to find midpoint, to use as possible impairment threshold
metrics.scores.cc1 <- metrics.scores %>% 
  filter(site_id %in% train.df.vec,
         condition_class == 1) 
quantile(metrics.scores.cc1$SITE_SCORE, probs = 0.25)

quantile(metrics.scores.cc1$SITE_SCORE, probs = seq(0, 1, 0.05), na.rm = FALSE, names = TRUE, type = 7)
...
metrics.scores.plot %>% 
  ...
  ylab("LGBAP Score") +
  geom_hline(yintercept=5.56, color = "red") +
  geom_hline(yintercept=4.37, color = "blue") +
  geom_hline(yintercept=4.8, color = "green") +
  geom_hline(yintercept=5.79, color = "purple")

  # ggsave(file.path(root.dir, "data/ibi/plots_for_pub", "ibi_training_final_2021-12-07.png"), dpi = "print")
...
metrics.scores.plot %>% 
  ...
  # ggsave(file.path(root.dir, "data/ibi/plots_for_pub", "ibi_val_final_2021-12-07.png"), dpi = "print")
...
metrics.scores.plot %>%
  ...
  xlab("Condition Class") +
  ylab("LGBAP Score") 
  # ggsave(file.path(root.dir, "data/ibi/plots_for_pub", "ibi_allsites_final_2021-12-07.png"), dpi = "print")
