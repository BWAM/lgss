...
condition <- read.csv(file.path("C:/Data/LGSS_scripting/lgss/data/compile_2015-2019/condition_classes", "2015-2019_LG_condclasses_20210608-reclass.csv")) %>%   
  select(-PC1)
...
# 
# write.csv(test.df, file.path(root.dir, "data/metrics/june2021_rerun/test_subsample_setseed70_allmetrics_2021-06-03.csv"), row.names = F)
# write.csv(train.df, file.path(root.dir, "data/metrics/june2021_rerun/train_subsample_setseed70_allmetrics_2021-06-03.csv"), row.names = F)
...
# write.csv(sens.filt, file.path(root.dir, "data/metrics/june2021_rerun/sens.filt_seed70_2021-06-07.csv"))
```
...
metrics.scores<-metrics.scores %>%
  group_by(site_id) %>%
  mutate(SITE_SCORE = sum(c(rich_ttaxa_insecta, dom_3_ttaxa, shannon_ttaxa_shredder, shannon_ttaxa_cote))) %>%
  mutate(SITE_SCORE = SITE_SCORE/4) %>%
  ungroup() %>%
  select(site_id, SITE_SCORE, rich_ttaxa_insecta, dom_3_ttaxa, shannon_ttaxa_shredder, shannon_ttaxa_cote)
...
#       0%      25%      50%      75%     100% 
# 25.17964 61.43169 70.52982 74.60222 99.85471 
...
quantile(metrics.scores.c3train$SITE_SCORE, probs = c(0, 0.25, 0.5, 0.75, 1))
 #      0%      25%      50%      75%     100% 
 # 0.00000 19.45799 33.00514 42.87084 69.97702 
...
metrics.scores.rescale <- metrics.scores %>% 
  mutate(SITE_SCORE_FINAL = (((SITE_SCORE - 19.45799)/(74.60222-19.45799)) * 10)) %>% 
