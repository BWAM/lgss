...
metrics.scores<-metrics.scores %>%
  ...
  mutate(SITE_SCORE = SITE_SCORE/6) %>%
  ungroup() %>%
...
metrics.scores %>% 
  ...
  coord_cartesian(ylim=c(0, 100))+
  geom_boxplot()
# ggsave(file.path(root.dir, "data/ibi/reruns", "lg_ibi_boxplot_set6_seed80new-val+class2_2021-05-19.png"), dpi = "screen")
...
#       0%      25%      50%      75%     100% 
# 48.50297 56.91925 72.97562 81.95297 93.81859 
...
quantile(metrics.scores.c3train$SITE_SCORE, probs = c(0, 0.25, 0.5, 0.75, 1))
 #       0%       25%       50%       75%      100% 
 # 4.928648 20.375112 32.322895 45.144131 65.285423 
...
metrics.scores.rescale <- metrics.scores %>% 
  mutate(SITE_SCORE_FINAL = (((SITE_SCORE - 20.375112)/(81.95297-20.375112)) * 10)) %>% 
