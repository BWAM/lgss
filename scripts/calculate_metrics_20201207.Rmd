...
write.csv(sens.filt, file.path(root.dir, "data/metrics/june2021_rerun/sens.filt_seed70_20210603.csv"))
```
...
metrics.scores<-metrics.scores %>%
  group_by(site_id) %>%
  mutate(SITE_SCORE = sum(c(dom_5_ttaxa, shannon_ttaxa_shredder, rich_ttaxa_tol_0to7, rich_ttaxa_toe, rich_ttaxa, shannon_ttaxa_toe))) %>%
  mutate(SITE_SCORE = SITE_SCORE/6) %>%
  ungroup() %>%
  select(site_id, SITE_SCORE, dom_5_ttaxa, shannon_ttaxa_shredder, rich_ttaxa_tol_0to7, rich_ttaxa_toe, rich_ttaxa, shannon_ttaxa_toe)
