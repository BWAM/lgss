...
taxa.df <- read_csv(file.path(root.dir, "data/compile_2015-2019/bugs", "2015-2019_lowgrad_taxa_unique_long_speciesfill_20210105.csv"))
...
### NEED TO RUN FURTHER SUBSAMPLES TO SEE WHICH METRICS COME OUT through filter. Then set final seed for training.
# set.seed(80)
set.seed(50)
...
# write.csv(sens.filt, file.path(root.dir, "data/metrics/sens.filt.20210105_b.csv"))
```
...
ggsave(file.path(root.dir, "data/ibi/reruns", "metrics_boxplots_set3_seed50_20210105.png"), dpi = "screen")
write_csv(sens.final, file.path(root.dir, "data/ibi/reruns/sens_final_seed50_20210105.csv"))
...
metrics.scores<-metrics.scores %>%
  group_by(site_id) %>%
  mutate(SITE_SCORE = sum(c(dom_5_ttaxa, rich_ttaxa_ept, rich_ttaxa_gatherer, rich_ttaxa_intol_facul, shannon_ttaxa_toe))) %>%
  mutate(SITE_SCORE = SITE_SCORE/5) %>%
  ungroup() %>%
  select(site_id, SITE_SCORE, dom_5_ttaxa, rich_ttaxa_ept, rich_ttaxa_gatherer, rich_ttaxa_intol_facul, shannon_ttaxa_toe)
...
metrics.scores %>% 
  ...
  coord_cartesian(ylim=c(0, 100))+
  geom_boxplot()
ggsave(file.path(root.dir, "data/ibi/reruns", "lg_ibi_boxplot_set3_seed50-val+class2.png"), dpi = "screen")
...
write_csv(train.df, file.path(root.dir, "data/ibi/reruns", "lg_ibi_training_seed50_20210105.csv"))
```
