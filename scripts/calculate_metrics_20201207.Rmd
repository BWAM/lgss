...
taxa.df <- read_csv(file.path(root.dir, "data/compile_2015-2019/bugs", "2015-2019_lowgrad_taxa_unique_long_speciesfill_2021-11-30.csv"))
...
# ~23% of taxa in this dataset do not have voltinism defined!
...
taxa.df.tol.na <- taxa.df.distinctsp %>% 
  filter(is.na(tol_char))
# ~7% of taxa in this dataset do not have voltinism defined.
...
metrics.thresholds <- metrics.df.standardized.long %>%
  ...
  #   disturbance == "decrease" ~ quantile(value, probs = 0.25),
  #   disturbance == "increase" ~ quantile(value, probs = 0.75)
  # )) %>%
  # mutate(ceiling = case_when(
  #   disturbance == "decrease" ~ quantile(value, probs = 0.75),
  #   disturbance == "increase" ~ quantile(value, probs = 0.25)
  # )) %>%
  ungroup() %>%
  ...
  # select(metric, disturbance,
         # floor = floor_3,
         # ceiling = ceiling_1)

# write_csv(metrics.thresholds, file.path(root.dir, "data/ibi/reruns", "metrics_thresholds_2021-07-27.csv"))
...
metrics.scores.plot %>% 
  ...
  mutate(condition_class = factor(class, c("LD", "SMD", "MD"))) %>% 
  ...
  ylab("LG-BAP Score") 
  # ggsave(file.path(root.dir, "data/ibi/plots_for_pub", "ibi_training_final_2021-07-30.png"), dpi = "print")
...
metrics.scores.plot %>% 
  ...
  # ggsave(file.path(root.dir, "data/ibi/plots_for_pub", "ibi_val_final_2021-07-30.png"), dpi = "print")
...
metrics.scores.plot %>%
  ...
  xlab("Condition Class") +
  ylab("LG-BAP Score") 
  # ggsave(file.path(root.dir, "data/ibi/plots_for_pub", "ibi_allsites_final_2021-07-30.png"), dpi = "print")
