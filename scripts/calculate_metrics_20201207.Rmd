...
div_subgr.df <- nest.df %>%
  mutate(
    ...
    simpson_ttaxa_potec = taxa_div(.data = .,
                            .key_col = site_id,
                            .group_col = ttaxa,
                            .counts_col = count,
                            .filter = order %in% c("coleoptera",
                            ...
                            .job = "simpson",
                            .unnest_col = data),
    simpson_ttaxa_cote = taxa_div(.data = .,
                            .key_col = site_id,
                            .group_col = ttaxa,
                            .counts_col = count,
                            .filter = order %in% c("coleoptera",
    ...
    simpson_ttaxa_cot = taxa_div(.data = .,
                            ...
                            .job = "simpson",
                            .unnest_col = data),
    simpson_ttaxa_toe = taxa_div(.data = .,
                            .key_col = site_id,
                            .group_col = ttaxa,
                            .counts_col = count,
                            .filter = order %in% c("odonata",
                            ...
                            .job = "simpson",
                            .unnest_col = data),
    simpson_ttaxa_coe = taxa_div(.data = .,
                            .key_col = site_id,
                            .group_col = ttaxa,
                            .counts_col = count,
                            .filter = order %in% c("coleoptera",
                            ...
                            .job = "simpson",
                            .unnest_col = data),
    simpson_ttaxa_cte = taxa_div(.data = .,
                            .key_col = site_id,
                            .group_col = ttaxa,
                            .counts_col = count,
                            .filter = order %in% c("coleoptera",
...
## Compile and remove underperforming metrics
...
# write.csv(sens.filt, file.path(root.dir, "data/metrics/sens.filt_seed50_20210106.csv"))
```
...
# metrics.final <- c("dom_5_ttaxa", "rich_ttaxa_ept", "rich_ttaxa_gatherer", "rich_ttaxa_intol_facul", "shannon_ttaxa_toe")
    # results in only 64 '0' metric scores. Better final IBI DE! Able to score more sites.
    # shannon_ttaxa_toe corr w rich_ttaxa_intol_facul at 0.77. But arguement to keep both, since different groups (one diversity, other tolerance/richness?)
    ### FINAL CHOSEN SET ###

# Final set #4 - Adding back in pct_rich_ttaxa_cote.
# metrics.final <- c("dom_5_ttaxa", "pct_rich_ttaxa_cote", "rich_ttaxa_ept", "rich_ttaxa_gatherer", "rich_ttaxa_intol_facul", "shannon_ttaxa_toe")
    # No significant difference. (68 '0' metrics scores)

# Final set #5 - Subbing in even more inclusing metric with slighly lower DE's.
# metrics.final <- c("dom_5_ttaxa", "rich_ttaxa", "rich_ttaxa_gatherer", "rich_ttaxa_intol_facul", "shannon_ttaxa_cote")
...
# metrics.final <- c("dom_5_ttaxa", "pct_gatherer", "rich_ttaxa", "rich_ttaxa_toe", "rich_ttaxa_intol_facul", "shannon_ttaxa_toe")
metrics.final <- c("dom_5_ttaxa", "pct_gatherer", "rich_ttaxa", "rich_ttaxa_toe", "rich_ttaxa_intol_facul", "shannon_ttaxa_toe", "pct_rich_ttaxa_cte")
...
# ggsave(file.path(root.dir, "data/ibi/reruns", "metrics_boxplots_set6_seed80new_20210106.png"), dpi = "screen")
...
Plot metrics against abiotic variables 
```{r}
...
# Only retain unique sites used in IBI dev (some were resampled in 2017-2019)
  ...
  select(-c(sample_date, uniqueid, "temperature of ph analysis", "conductivity at 25 degrees celsius", "silver", "ph", "chlorophyll a",
...
train.df.cor <- train.df.final.corr %>% 
  select(-condition_class)


# Abiotic param correlations w metrics
metrics.cor.abiotic<-left_join(train.df.cor, abiotic, by=c("site_id"))
metric.param.corr<-metrics.cor.abiotic %>% 
  select(-site_id) %>% 
  correlate(use = "pairwise.complete.obs", method = "spearman")
metric.param.corr<-metric.param.corr %>% 
  select(term, dom_5_ttaxa, pct_gatherer, rich_ttaxa, rich_ttaxa_toe, rich_ttaxa_intol_facul, shannon_ttaxa_toe, pct_rich_ttaxa_cte) %>%
  slice(8:14) %>% 
  remove_rownames %>% 
  column_to_rownames(var="term")  
...
# Chemisty param correlations w metrics
metrics.cor.chem<-left_join(train.df.cor, abiotic.chem, by=c("site_id"))
...
# Truncate Scores >100 to 100 and <0 to 0
metrics.df.standardized.long.percentiles$SCORE<-ifelse(metrics.df.standardized.long.percentiles$SCORE > 100, 100, metrics.df.standardized.long.percentiles$SCORE)
...
# Metrics set 3 (FINAL CHOSEN SET)
metrics.scores<-metrics.scores %>%
  group_by(site_id) %>%
  mutate(SITE_SCORE = sum(c(dom_5_ttaxa, pct_gatherer, rich_ttaxa, rich_ttaxa_toe, rich_ttaxa_intol_facul, shannon_ttaxa_toe))) %>%
  mutate(SITE_SCORE = SITE_SCORE/5) %>%
  ungroup() %>%
  select(site_id, SITE_SCORE, dom_5_ttaxa, pct_gatherer, rich_ttaxa, rich_ttaxa_toe, rich_ttaxa_intol_facul, shannon_ttaxa_toe)
 
# Alternative metrics set
# metrics.scores<-metrics.scores %>% 
#   group_by(site_id) %>% 
#   mutate(SITE_SCORE = sum(c(dom_5_ttaxa, rich_ttaxa_et, rich_ttaxa_gatherer, rich_ttaxa_intolerant, shannon_ttaxa_toe))) %>% 
#   mutate(SITE_SCORE = SITE_SCORE/5) %>% 
#   ungroup() %>% 
#   select(site_id, SITE_SCORE, dom_5_ttaxa, rich_ttaxa_et, rich_ttaxa_gatherer, rich_ttaxa_intolerant, shannon_ttaxa_toe)
...
# Plot validation + class 2 (all data minus training)
metrics.scores %>% 
  ...
  coord_cartesian(ylim=c(0, 100))+
  geom_boxplot()
# ggsave(file.path(root.dir, "data/ibi/reruns", "lg_ibi_boxplot_set6_seed80new-val+class2.png"), dpi = "screen")
...
# write_csv(train.df, file.path(root.dir, "data/ibi/reruns", "lg_ibi_training_seed80new_20210106.csv"))
```
