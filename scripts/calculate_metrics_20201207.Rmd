...
# sens.range.long <- pivot_longer(sens.df.subset.range, cols = summary_stat, names_to = "metric", values_to = "value")
# 
# sens.df.subset.range.long <- gather(sens.df.subset.range, metric, value)
#                                            # rich_ttaxa_toe:simpson_ttaxa_baetidae)
# write.csv(sens.df.subset.range, "sens.ranges.csv")
...
### ASK MATT, WORK ON UNDERSTANDING
...
#... SOI >=2  
### Ask Matt [if/why] he excluded these?
# pct_ep, pct_ephemeroptera, pct_furcatergalia, pct_baetidae, pct_anisoptera, pct_dubiraphia, pct_pisciforma, pct_toe, pct_elmidae, pct_et, pct_tanytarsus, pct_cote, pct_ept, pct_potec, pct_pt


# 43 unique above, brings total from 62 down to 19.

# SOI (>2): ...Ask Brian/Matt about this
...
# Additional redundant metrics to remove? (high correlations)
# either pct_et or pct_ept
# either pct_rich_ttaxa_potec or pct_rich_ttaxa_cote
# either pct_potec or pct_cote
# either rich_ttaxa or margalef_ttaxa


## LOOK AT SCATTERPLOTS OF THOSE W/ CORR'S ABOVE THRESHOLD?
...
# Get list of filtered metrics at this point
metrics.filt <- train.df.final %>% 
  select(-c(site_id, condition_class)) %>% 
  names()

sensitivity.final <- sensitivity.df %>% 
...
```{r View sens table for final metrics, eval=FALSE}
DT::datatable(head(sens.final, 500), options = list(scrollX = TRUE))

```



Traditional method: Standardize metrics
