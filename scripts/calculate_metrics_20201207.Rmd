...
Calculated as the ratio of the range of possible impairment (values beyond the poorer quality 25th percentile) and the IQR of reference sites, a measure of variability. Relative SOI values larger than 1 are more desirable, with values less than 1 indicating too much variability among reference sites compared to the range of impairment. (Blocksom 2009)
```{r SOI}
...
# [incomplete] code failed metrics as vector so don't have to list out metrics for removal later.
...
```{r View sens table for filtered metrics, eval=FALSE}
sens.filt <- sens.df %>% 
  filter(metric %in% metrics.filt)
...
# Outputs weighting coefficients of metrics used... margalef_ttaxa heavily weighted!!!
