...
# To install mmir package:
# install.packages("pak")
# pak::pak("zsmith27/mmir")
...
# Look at trichop only
taxa.df.class.trichop <- taxa.df.class %>% filter(order %in% "trichoptera")


taxa.df.class.count <- taxa.df.class %>% 
  count(site_id, order, condition_class) %>% 
  pivot_wider(names_from = "order", values_from = "n", values_fill = 0) %>% 
  relocate(site_id, condition_class, ephemeroptera, plecoptera, trichoptera, coleoptera, odonata)

# roll up by class to look at abundance of non-insect organisms...
taxa.df.class.count.class <- taxa.df.class %>% 
  count(site_id, class, condition_class) %>% 
  pivot_wider(names_from = "class", values_from = "n", values_fill = 0) %>% 
  relocate(site_id, condition_class)

taxa.df.class.count.class.LD <- taxa.df.class.count.class %>% 
  filter(condition_class == "1") %>% 
  janitor::adorn_totals()

taxa.df.class.count.class.MD <- taxa.df.class.count.class %>% 
...
set.seed(60) # USED FOR FINAL RUN. RERAN 9/16/2025 and got the same training and test sites :)
# set.seed(70)
# set.seed(80)
# set.seed(90)
# set.seed(5122)
...
metric.param.corr.flip <- metric.param.corr %>% 
  t()

# ggsave(file.path(root.dir, "data/metrics/abiotic_corr", "metrics_abiotic_corr_2022-02-23.png"), dpi = "print")
...
# ggsave(file.path(root.dir, "data/metrics/abiotic_corr", "metrics_abiotic_corr_chem_2022-02-23.png"), dpi = "print")
...
# Chemistry param correlations w metrics (validation only; 9 SITES ONLY)
...
```{r Standardize - Method 3 ~FINAL CHOSEN METHOD~ (centering)}
...
# Determine standard deviations and Median Absolute Deviations for each metric (training)
...
# Replot (training only)
metrics.centered.scale.train <- metrics.centered.scale %>% 
   filter(site_id %in% train.df.vec,
         condition_class != 2)  
ggplot(metrics.centered.scale.train, aes(x = as.character(condition_class), y = rescaled)) +
  geom_boxplot() +
  facet_wrap(~metric)
...
# Export DFs for running this chunk in a separate file
# dir.ibiplot_dfs <- file.path(here::here(), "data/IBI/df_exports_for_IBI_plot")
# write_csv(metrics.scores, file.path(dir.ibiplot_dfs, "metrics.scores.df_2022-12-21.csv"))
# write.csv(train.df.vec, file.path(dir.ibiplot_dfs, "train.df.vec_2022-12-21.csv"), row.names = F)
# write.csv(test.df.vec, file.path(dir.ibiplot_dfs, "test.df.vec_2022-12-21.csv"), row.names = F)
...
```{r Chem vs IBI plot, fig.width=14, fig.height=12}
# Chemistry (independent dataset) correlations w metrics (all sites (44), final metrics)
...
# Rerun IBI-chem correlations using psych package to get p-values for reporting (in lieu of using Dunn's test below)
ibi.chem.corrtest <- metrics.cor.chem %>% 
  select(-1, -c(3:7)) %>% 
  psych::corr.test(., method = "spearman", adjust = "none")
ibi.chem.corrtest.r <- as.data.frame(ibi.chem.corrtest$r)
ibi.chem.corrtest.p <- as.data.frame(ibi.chem.corrtest$p)

# ibi.chem.corrp2 <- metrics.cor.chem %>% 
#   select(-1, -c(3:7)) %>% 
#   psych::corr.p(., 42, adjust = "none")

# Scatterplots of chem vs IBI scores
metrics.cor.chem.melt <- metrics.cor.chem %>% 
  select(-1, -c(3:7)) %>% 
  reshape2::melt(., id = "LGBAP")

ggplot(metrics.cor.chem.melt, aes(x = LGBAP, y = value)) +
    facet_wrap(~variable, scales = "free") +
    geom_point() +
  geom_smooth(method = "lm")


# Perform Dunn's test of multiple comparisons
chem.test <- chemsites.ibi %>% 
  select(site_id, LGBAP) %>% 
  left_join(abiotic.chem) %>% 
  pivot_longer(cols = c(-site_id, -LGBAP),
               names_to = "param",
               values_to = "value") %>% 
  mutate(classification = case_when(
    ...
    LGBAP <= 5.0  ~ "moderate",
    LGBAP <= 7.5  ~ "slight",
    LGBAP > 7.5 ~ "non-impacted"
  ))

library(dunn.test)
dunn_test.chem <- chem.test %>%
  group_by(param) %>%
  nest() %>%
  mutate(model = map(data, ~dunn.test(x = .x$value, g = .x$classification, method = "bonferroni") %>% as_tibble())) %>%
  unnest(model)

p <- chem.test %>% 
  mutate(classification = factor(classification, c("non-impacted", "slight", "moderate", "severe"))) %>%  
  ggplot(aes(classification, value)) +
  geom_boxplot(outlier.size = 0.7) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1), axis.title.y=element_blank())

p + facet_wrap(vars(param), ncol = 6, scales = "free", strip.position = "left",)


```
```{r Kruskal-Wallis tests on IBI vs certain chem params}

chem.test.binary <- chemsites.ibi %>% 
  select(site_id, LGBAP) %>% 
  left_join(abiotic.chem) %>% 
  pivot_longer(cols = c(-site_id, -LGBAP),
               names_to = "param",
  ...
  mutate(classification = case_when(
    LGBAP < 5 ~ "impaired",
    LGBAP >= 5 ~ "non-impacted"
  ))

chem.test.cl <- chem.test.binary %>% 
  filter(grepl("chloride", param))
shapiro.test(chem.test.cl$value)
wilcox.test(chem.test.cl$value ~ chem.test.cl$classification)
kruskal.test(chem.test.cl$value ~ chem.test.cl$classification)





chem.test.cl <- chem.test %>% 
  filter(grepl("ammonia", param))
kruskal.test(chem.test.cl$value ~ chem.test.cl$classification)
chem.test.TN <- chem.test %>% 
  filter(param %in% "nitrogen_mg/l")
kruskal.test(chem.test.TN$value ~ chem.test.TN$classification)
chem.test.TP <- chem.test %>% 
  filter(param %in% "phosphorus, total (as p)_mg/l")
kruskal.test(chem.test.TP$value ~ chem.test.TP$classification)
chem.test.cu <- chem.test %>% 
  filter(grepl("copper", param))
kruskal.test(chem.test.cu$value ~ chem.test.cu$classification)
```

```{r Abiotic vs IBI, fig.width=14, fig.height=3}

scores.val <- metrics.scores %>%
  select(-condition_class) %>%
  filter(site_id %in% test.df.vec) %>% 
  rename(LGBAP = SITE_SCORE)
...
library(dunn.test)
dunn_test.abiotic <- abiotic.test %>%
  group_by(param) %>%
  nest() %>%
  mutate(model = map(data, ~dunn.test(x = .x$value, g = .x$classification, method = "bonferroni") %>% as_tibble())) %>%
  unnest(model)
...
p + facet_wrap(vars(param), ncol = 7, scales = "free", strip.position = "left",)


# Perform simple Kruskal-Wallis tests on binary bins

abiotic.test.binary <- scores.val %>% 
  select(site_id, LGBAP) %>% 
  left_join(abiotic) %>% 
  pivot_longer(cols = c(-site_id, -LGBAP),
               names_to = "param",
...
## Below code does not work. Attempted to create table of KW tests similar to Dunn test table above.
# kw_test.abiotic <- abiotic.test %>%
#   group_by(param) %>%
#   nest() %>%
#   mutate(model = map(data, ~kruskal.test(.x$value ~ .x$classification) %>% as_tibble())) %>%
#   unnest(model)

abiotic.test.spcond <- abiotic.test.binary %>% 
  filter(grepl("spcond", param))
shapiro.test(abiotic.test.spcond$value)
wilcox.test(abiotic.test.spcond$value ~ abiotic.test.spcond$classification)
kruskal.test(abiotic.test.spcond$value ~ abiotic.test.spcond$classification)

abiotic.test.nat <- abiotic.test.binary %>% 
  filter(grepl("nat", param))
shapiro.test(abiotic.test.nat$value)
wilcox.test(abiotic.test.nat$value ~ abiotic.test.nat$classification)
kruskal.test(abiotic.test.nat$value ~ abiotic.test.nat$classification)

abiotic.test.dev <- abiotic.test.binary %>% 
  filter(grepl("dev", param))
shapiro.test(abiotic.test.dev$value)
wilcox.test(abiotic.test.dev$value ~ abiotic.test.dev$classification)
kruskal.test(abiotic.test.dev$value ~ abiotic.test.dev$classification)

abiotic.test.hab <- abiotic.test.binary %>% 
  filter(grepl("habitat", param))
shapiro.test(abiotic.test.hab$value)
wilcox.test(abiotic.test.hab$value ~ abiotic.test.hab$classification)
kruskal.test(abiotic.test.hab$value ~ abiotic.test.hab$classification)

abiotic.test.ag <- abiotic.test.binary %>% 
  filter(grepl("ag", param))
shapiro.test(abiotic.test.ag$value)
wilcox.test(abiotic.test.ag$value ~ abiotic.test.ag$classification)
kruskal.test(abiotic.test.ag$value ~ abiotic.test.ag$classification)

abiotic.test.ph <- abiotic.test.binary %>% 
  filter(param %in% "ph")
shapiro.test(abiotic.test.ph$value)
kruskal.test(abiotic.test.ph$value ~ abiotic.test.ph$classification)
summary(aov(abiotic.test.ph$value ~ abiotic.test.ph$classification))

abiotic.test.mac <- abiotic.test.binary %>% 
  filter(grepl("macrophyte", param))
shapiro.test(abiotic.test.mac$value)
wilcox.test(abiotic.test.mac$value ~ abiotic.test.mac$classification)
kruskal.test(abiotic.test.mac$value ~ abiotic.test.mac$classification)
...
```{r Data prep and model creation, eval=FALSE}
...
```{r Cross validation and measuring performace, eval=FALSE}
...
```{r Create logistical regression model, eval=FALSE}
...
```{r Logistical regression model plots, eval=FALSE}
...
```{r Visualize model performance (ROC curve), eval=FALSE}
...
```{r, confusion matrix accuracy assessment for validation dataset, eval=FALSE}
