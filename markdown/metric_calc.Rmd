...
`taxa_rich()` is used to calculate richness. A long format data frame containing taxonomic counts is specified as the `long.df` variable (`lgss.df`). The name of a unique ID column is specified for the `unique.id.col` variable (`unique_id`). Note that this is done with NSE syntax (i.e., no quotes). The `low.taxa.col` and `high.taxa.col` refer to low resolution and high resolution taxonomic rank columns of interest. `high.taxa.col` is required but `low.taxa.col` is only necessary when calculating subgroup richness. Furthermore, the `taxon` variable is only necessary for specifying which taxon or taxa will be used to calculate subgroup richness; therefore, when calculating community richness, `taxon` should be set to `NULL`. In this example family-level and genus-level community richness are calculated and appended to `metrics.key`. `DT::datatable()` is only used to present interactive tables in this document.
```{r}
rich.df <- metrics.key %>% 
  dplyr::mutate(
    rich_order = taxa_rich(lgss.df, unique_id, class, order, taxon = NULL),
    rich_family = taxa_rich(lgss.df, unique_id, order, family, taxon = NULL),
    rich_genus = taxa_rich(lgss.df, unique_id, order, genus, taxon = NULL)
  )
...
`taxa_rich()` can also be used to calculate subgroup richness, which refers to calculating the richness of only select set of taxa. In the example below Ephemeroptera (Mayfly) richness is calculated at the family- and genus-levels. More than one taxonomic group can be specified, as shown for `rich_ept_gen` (`taxon = c("ephemeroptera", "plecoptera", "trichoptera")`). The `taxon` variable is used to filter the data frame based on character string matches found in specified `low.taxa.col`. In the example, the `lgss.df` data frame is subset to only include rows where the `order` column specifies "ephemeroptera". The `high.taxa.col` (e.g., `family` or `genus`) is then used to find the number of unique ephemeroptera taxa at this specified taxonomic level. If `taxon` was set to "baetidae" (a family of ephmeroptera) in the example below, then `taxa_rich()` would return all zeros because the character string "baetidae" would never be found in the order-level column.
```{r}
subrich.df <- metrics.key %>% 
  dplyr::mutate(
    rich_ephemeroptera_fam = taxa_rich(lgss.df, unique_id, order, family, taxon = "ephemeroptera"),
    rich_ephemeroptera_gen = taxa_rich(lgss.df, unique_id, order, genus, taxon = "ephemeroptera"),
    rich_ept_fam = taxa_rich(lgss.df, unique_id, order, family,
                             taxon = c("ephemeroptera", "plecoptera", "trichoptera")),
    rich_ept_gen = taxa_rich(lgss.df, unique_id, order, genus,
                             taxon = c("ephemeroptera", "plecoptera", "trichoptera")),
    rich_cote_fam = taxa_rich(lgss.df, unique_id, order, family,
                             taxon = c("coleoptera", "odonata", "trichoptera", "ephemeroptera")),
    rich_cote_gen = taxa_rich(lgss.df, unique_id, order, genus,
                             taxon = c("coleoptera", "odonata", "trichoptera", "ephemeroptera")),
    rich_potec_fam = taxa_rich(lgss.df, unique_id, order, family,
                             taxon = c("plecoptera", "odonata", "coleoptera",  "trichoptera", "ephemeroptera")),
    rich_potec_gen = taxa_rich(lgss.df, unique_id, order, genus,
                             taxon = c("coleoptera", "odonata", "trichoptera", "ephemeroptera")),
  )
...
div.df <- metrics.key %>% 
  dplyr::mutate(
    shannon_genus = taxa_div(lgss.df, unique_id, indiv,
                              high.taxa.col = genus, job = "shannon", base.log = 2),
    simpson_genus = taxa_div(lgss.df, unique_id, indiv,
                              high.taxa.col = genus, job = "simpson"),
    margalef_genus = taxa_div(lgss.df, unique_id, indiv,
                               high.taxa.col = genus, job = "margalef"),
    menhinick_genus = taxa_div(lgss.df, unique_id, indiv,
                                high.taxa.col = genus, job = "menhinick"),
    pielou_genus = taxa_div(lgss.df, unique_id, indiv,
...
pct.df <- metrics.key %>% 
  dplyr::mutate(
    pct_ephemeroptera = taxa_pct(long.df = lgss.df,
                                  unique.id.col = unique_id,
                                  count.col = indiv,
                                  taxon.col = order,
                                  taxon = "ephemeroptera"),
    pct_ept = taxa_pct(lgss.df,
                        unique.id.col = unique_id,
                        count.col = indiv,
                        taxon.col = order,
                        taxon = c("ephemeroptera", "plecoptera", "trichoptera"))
...
```{r}
abund.df <- metrics.key %>% 
  dplyr::mutate(
    abund_ephemeroptera = taxa_abund(lgss.df,
                                  unique.id.col = unique_id,
                                  count.col = indiv,
                                  taxon.col = order,
                                  taxon = "ephemeroptera"),
    abund_ept = taxa_abund(lgss.df,
                        unique.id.col = unique_id,
                        count.col = indiv,
                        taxon.col = order,
                        taxon = c("ephemeroptera", "plecoptera", "trichoptera"))
...
seq.df <- metrics.key %>% 
  bind_cols(
    taxa_seq(lgss.df, unique_id, indiv, c("order", "family"), "genus",  job = "rich"),
    taxa_seq(lgss.df, unique_id, indiv, c("order", "family"), "genus",  job = "pct_rich"),
    taxa_seq(lgss.df, unique_id, indiv, c("order", "family"),  job = "pct"),
    taxa_seq(lgss.df, unique_id, indiv, c("order", "family"),  job = "abund")
  )
...
metrics.wide <-  list(rich.df,
                      ...
                      seq.df) %>% 
  plyr::join_all(by = c("unique_id", "condition"))
```
