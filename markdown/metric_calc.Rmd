...
subrich.df <- metrics.key %>% 
  dplyr::mutate(
    rich_ephemeroptera_fam = taxa_rich(taxa.df, newid, order, family, taxon = "ephemeroptera"),
    rich_ephemeroptera_gen = taxa_rich(taxa.df, newid, order, genus, taxon = "ephemeroptera"),
    rich_ept_fam = taxa_rich(taxa.df, newid, order, family,
                             taxon = c("ephemeroptera", "plecoptera", "trichoptera")),
    rich_ept_gen = taxa_rich(taxa.df, newid, order, genus,
                             taxon = c("ephemeroptera", "plecoptera", "trichoptera")),
    rich_cote_fam = taxa_rich(taxa.df, newid, order, family,
                             taxon = c("coleoptera", "odonata", "trichoptera", "ephemeroptera")),
    rich_cote_gen = taxa_rich(taxa.df, newid, order, genus,
                             taxon = c("coleoptera", "odonata", "trichoptera", "ephemeroptera")),
    rich_potec_fam = taxa_rich(taxa.df, newid, order, family,
                             taxon = c("plecoptera", "odonata", "coleoptera",  "trichoptera", "ephemeroptera")),
    rich_potec_gen = taxa_rich(taxa.df, newid, order, genus,
                             taxon = c("coleoptera", "odonata", "trichoptera", "ephemeroptera")),
  )
...
div.df <- metrics.key %>% 
  dplyr::mutate(
    shannon_genus = taxa_div(taxa.df, newid, indiv,
                              high.taxa.col = genus, job = "shannon", base.log = 2),
    simpson_genus = taxa_div(taxa.df, newid, indiv,
                              high.taxa.col = genus, job = "simpson"),
    margalef_genus = taxa_div(taxa.df, newid, indiv,
                               high.taxa.col = genus, job = "margalef"),
    menhinick_genus = taxa_div(taxa.df, newid, indiv,
                                high.taxa.col = genus, job = "menhinick"),
    pielou_genus = taxa_div(taxa.df, newid, indiv,
...
pct.df <- metrics.key %>% 
  dplyr::mutate(
    pct_ephemeroptera = taxa_pct(long.df = taxa.df,
                                  unique.id.col = newid,
                                  count.col = indiv,
                                  taxon.col = order,
                                  taxon = "ephemeroptera"),
...
abund.df <- metrics.key %>% 
  dplyr::mutate(
    abund_ephemeroptera = taxa_abund(taxa.df,
                                  unique.id.col = newid,
                                  count.col = indiv,
                                  taxon.col = order,
                                  taxon = "ephemeroptera"),
    abund_ept = taxa_abund(taxa.df,
                        unique.id.col = newid,
                        count.col = indiv,
                        taxon.col = order,
                        taxon = c("ephemeroptera", "plecoptera", "trichoptera"))
...
seq.df <- metrics.key %>% 
  bind_cols(
    taxa_seq(taxa.df, newid, indiv, c("order", "family"), "genus",  job = "rich"),
    taxa_seq(taxa.df, newid, indiv, c("order", "family"), "genus",  job = "pct_rich"),
    taxa_seq(taxa.df, newid, indiv, c("order", "family"),  job = "pct"),
    taxa_seq(taxa.df, newid, indiv, c("order", "family"),  job = "abund")
  )
...
metrics.wide <-  list(rich.df,
                      ...
                      seq.df) %>% 
  plyr::join_all(by = c("newid", "condition"))
```
