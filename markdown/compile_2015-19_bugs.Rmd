...
# Invesigate issue taxa
# bugmatrix.15.16.17.df.sub <- bugmatrix.15.16.17.df %>% 
#   select(SITE_ID, COLL_DATE, REPLICATE, "Cricotopus/Orthocladius complex", "Cricotopus/Orthocladius Complex")
...
bugnames.15to19.mismatch <- bugnames.15to19 %>% 
  filter(!(genspecies %in% master.taxa$MMS_MACRO_GENSPECIES))

### nigronia_sp. is only one that doesn't exist in master taxa table ###
# See how many exist...
bugmatrix.15to19.nigronia <- bugmatrix.15to19 %>% 
  select(site_id, sample_date, nigronia)

bugmatrix.15to19.anafroptilum <- bugmatrix.15to19 %>% 
  select(site_id, sample_date, anafroptilum)
# Only 2 total individuals present across 2 sites.
...
# All bugs
write.table(bugmatrix.15to19, file.path(root.dir, "data/compile_2015-2019/bugs/2015-2019_lowgrad_bugs_matrix_20201202.csv"),sep=",", row.names = FALSE)
write.table(bugmatrix.15to19, file.path(teams.datadir, "bugs/2015-2019_lowgrad_bugs_matrix_20201202.csv"),sep=",", row.names = FALSE)

# Only unique bugs
write.table(bugmatrix.15to19.unique, file.path(root.dir, "data/compile_2015-2019/bugs/2015-2019_lowgrad_bugs_matrix_UNIQUE_20201202.csv"),sep=",", row.names = FALSE)
write.table(bugmatrix.15to19.unique, file.path(teams.datadir, "bugs", "2015-2019_lowgrad_bugs_matrix_UNIQUE_20201202.csv"),sep=",", row.names = FALSE)
