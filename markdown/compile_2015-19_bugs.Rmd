...
sites.master <- read_csv(file.path(root.dir, "data/compile_2015-2019", "20191224_Site_Field_cleaned_final.csv"))
...
# #Transform from long to wide (old code used on raw data)
# bugmatrix.18.19.df <- bugs.bind.18.19 %>% 
#   mutate(REPLICATE = ifelse(is.na(REPLICATE), 1, REPLICATE)) %>%
#   mutate(INDIV = as.numeric(INDIV)) %>% 
#   mutate(COLL_DATE = as.Date(COLL_DATE, format = "%m/%d/%Y")) %>% 
...
Check full bug list against master taxa table and reformat/update accordingly
```{r}
...
Clean up dataframes and export matrix
```{r}

rm(matchlist, bugmatrix.15.16.17.df, bugmatrix.18.19.df, bugs.2018.1, bugs.2018.2, bugs.2019, sites.master)

# write.table(bugmatrix.15to19, file.path(root.dir, "data/compile_2015-2019/bugs/NYSDEC_LowGrad_macro_2015-2019_2020-04-09.csv"),sep=",", row.names = FALSE)
write.table(bugmatrix.15to19, file.path(root.dir, "data/compile_2015-2019/bugs/2015-2019_lowgrad_bugs_matrix_20201030.csv"),sep=",", row.names = FALSE)
