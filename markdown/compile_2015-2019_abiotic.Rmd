...
field.LU.2015 <- read_excel(fielddata.2015.path, sheet = "FieldData") %>% 
  ...
  janitor::clean_names() %>% 
  rename_all(tolower) %>% 
  select(-c(name, direct, descript, lat_final, long_final)) %>% 
...
field.LU.2016.2017 <- read_excel(fielddata.2016.2017.path, sheet = "LG data + sites, LU, SteamStats") %>%  
  ...
  select(-c(location, rivmile, station, org_for_toxics, project, samp, secchi)) %>% 
...
Join 2015-2017 field and habitat, bind with master 2018-2019 table, then add in additional site info.
```{r}
...
write.table(master.field, file.path(root.dir, "data", "compile_2015-2019", "field_data", "final_cleaned", "2015-2019_lowgrad_fielddata_20201101.csv"),sep=",", row.names = FALSE)

sites <- read_excel(file.path(itstable.path, "20201030_S_SITE.xlsx")) %>% 
  rename_all(tolower) %>% 
  setNames(., sub("site_", "", names(.))) %>%
  rename(site_id = history_id) %>%
  filter(site_id %in% master.field$site_id)

missing_sites <- anti_join(sites, master.field, by = "site_id")
```
...
# Old approach (unfinished):


MACRO FIELD table: Filter to event list and add in 2015-2017 data
```{r}
# db.macrofield <- read_excel(file.path(itstable.path, "20201023_S_MACRO_FIELD_DATA_HISTORY_all_fields.xlsx"), guess_max = min(10000, n_max = NULL)) %>% 
db.macrofield <- read.csv(file.path(itstable.path, "20201023_S_MACRO_FIELD_DATA_HISTORY_all_fields.csv"), stringsAsFactors = FALSE) %>% 
  rename_all(tolower) %>% 
  setNames(., sub("mfdh_", "", names(.))) %>%
...
# list unique values in all fields to check import
db.macrofield.unique <- lapply(db.macrofield, unique)

db.macrofield.select <- db.macrofield %>% 
  select(-c(row_index, instance_id, location, rivermile, biosample_collect_method, biosamp_col_method_note))
field.master <- bind_rows(db.macrofield.select, field.LU.bind)

# setdiff(db.macrofield.select, field.LU.bind)


# Clean up unique names
macro.field.bind.unique <- lapply(field.master, unique)





rm(db.macrofield, field.LU.bind, db.macrofield.select)
```

SAMPLE EVENT INFO table: Filter to event list and add in 2015-2017 data
```{r}
# db.SEI <- read_excel(file.path(itstable.path, "20201019_S_SAMPLE_EVENT_INFO_all_fields.xlsx"), guess_max = min(10000, n_max = NULL)) %>% 
db.SEI <- read.csv(file.path(itstable.path, "20201019_S_SAMPLE_EVENT_INFO_all_fields.csv"), stringsAsFactors = FALSE) %>% 
  rename_all(tolower) %>% 
  setNames(., sub("seih_", "", names(.))) %>% 
  rename(event_id = event_smas_id,
  ...
  #        cur_spd = as.numeric(cur_spd))

db.SEI.select <- db.SEI %>% 
  select(event_id, arrivetime, crew, depth, width, cur_spd, algae_diaoms_presen, thickness_diatoms_present, macrophyte_pct, canopy, embeddedness, rock, rubble, gravel, sand, silt, clay, land_use, general_comment)

field.master2 <- left_join(field.master, db.SEI.select)

# field.master2 <- bind_rows(field.master, db.SEI.select)
