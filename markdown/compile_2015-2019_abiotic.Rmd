...
Read in current version of SBU DB tables, filter to LG 2018-2019, and format as needed
```{r}
# Read in bug data to get list of events for gathering associated data
...
### Get surveys that match bug sample event_ids
...
Get list of 2018-2019 LG sites from Habitat survey and cross-check against 2015-2019 bug matrix from compile_2015-2019_bugs.Rmd
# Clean up this chunk after using pieces for above
```{r}

# Get list of 2018-2019 LG sites from the macro field survey. Expecting 39 based on output created in compile_2015-2019_bugs.Rmd


# List all 2018-2019 LG macro field surveys present
db.macrofield.LGlist.18.19 <-  db.macrofield %>% 
  select(event_smas_id, site_id, sample_date, biosample_collect_method, habitat, lgs_habitat_bank, lgs_habitat_macrophyte, lgs_habitat_snag, lgs_habitat_substrate, dominant_substrate) %>% 
  mutate(sample_date = as.Date(sample_date, format = "%m/%d/%Y"),
         year = format(sample_date, format = "%Y")) %>% 
  filter(year %in% c("2018", "2019"),
         biosample_collect_method %in% "Low-Gradient") %>%
...
missingbugs_raw <- anti_join(db.macrofield.LGlist.17.18, bugs.bind.18.19.sitedate)
missingbugs_macrofield <- anti_join(bugs.bind.18.19.sitedate, db.macrofield.LGlist.17.18)

# See what bugs are present in the ITS tables
...
missingbugs_raw <- anti_join(db.mssih.LG1819, bugs.bind.18.19.sitedate)
# All 39 samples present in 2018-2019 raw files are present in DB!

# Get list of 2018-2019 LG sites using habitat survey. Expecting 39 based on output created in compile_2015-2019_bugs.Rmd
...
# List ALL LG habitat surveys present
# db.habitat.LGlist <-  db.habitat %>%
#   select(site_id, sample_date, gradient, habitat_comment, event_smas_id) %>% 
#   mutate(sample_date = as.Date(sample_date, format = "%m/%d/%Y"),
#          year = format(sample_date, format = "%Y")) %>% 
#   filter(gradient %in% "Low")

# > unique(db.habitat.LGlist$year)
#  [1] "2008" "2009" "2010" "2011" "2012" "2013" "2014" "2015" "2016" "2017" "2018" "2019"

# List all 2018-2019 LG habitat surveys present
db.habitat.LGlist.17.18 <-  db.habitat %>% 
  select(site_id, sample_date, gradient, habitat_comment, event_smas_id) %>% 
  mutate(sample_date = as.Date(sample_date, format = "%m/%d/%Y"),
         year = format(sample_date, format = "%Y")) %>% 
  filter(year %in% c("2018", "2019"),
...
# 57 presesnt! 18 possible missing from bugs (not sampled?). 

# Get list of bug samples from 2018-2019 bug matrix compiled in compile_2015-2019_bugs.Rmd and cross-check against 2018-2019 list here
bugs.bind.18.19.sitedate <- bugs.bind.18.19 %>% 
  rename_all(tolower) %>% 
...
missingbugs_hab <- anti_join(bugs.bind.18.19.sitedate, db.habitat.LGlist.17.18)
...
Check sites list against master sites table
```{r}
db.sites <- read_excel(file.path(itstable.path, "20201023_S_SITE.xlsx")) 
