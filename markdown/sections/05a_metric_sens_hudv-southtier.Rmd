...
DT::datatable(sw.sens.df.hvst, options = list(scrollX = TRUE))
```
...
sw.sens.df.hvst %>% 
  # filter(metric %in% metrics.vec2.hvst) %>%
  filter(barbour > 1) %>%
  filter(de >= 70) %>%
  inner_join(reg.df.hvst, by = "metric") %>% 
  arrange(desc(de)) %>% 
  mutate(metric = factor(metric, levels = unique(metric))) %>% 
  ggplot(aes(site_condition, value, fill = site_condition)) +
...
# ggsave(filename = "HV+ST_metrics_2_export_color.png", width = 7, height = 1.5, units = "in", path = 'C:/Users/gmlemley/OneDrive - New York State Office of Information Technology Services/Projects/LGSS/NAB, NEAEB/Figs/Box plots')
```

Final chosen metrics
```{r}
metrics.vec.hvst <- c("pct_et",
                 "pct_rich_cote_family",
                 "pct_filterer",
                 "shannon_genus"
                 )

metrics.vec2.hvst <- c("pct_rich_toe_family",
                  "pct_filterer",
                  "shannon_genus",
              		"pct_rich_mol_genus",
                  "pct_dom5_genus"
...
```{r}
sub.df.hvst <- reg.df.hvst %>% 
  filter(metric %in% metrics.vec.hvst) %>% 
  left_join(sw.sens.df.hvst, by = "metric") %>% 
  group_by(metric) %>% 
  mutate(quant_5 = quantile(value, 0.05),
         ...
         score = if_else(disturbance == "increase", 100 - score, score)) %>% 
  select(-min, -max, -value)

sub.df2.hvst <- reg.df.hvst %>% 
  filter(metric %in% metrics.vec2.hvst) %>% 
  left_join(sw.sens.df.hvst, by = "metric") %>% 
  group_by(metric) %>% 
  mutate(quant_5 = quantile(value, 0.05),
...
# ggsave(filename = "HV+ST_IBI_2_export.png", width = 3, height = 2.5, units = "in", path = 'C:/Users/gmlemley/OneDrive - New York State Office of Information Technology Services/Projects/LGSS/NAB, NEAEB/Figs/Box plots')
...
```{r}
selected.df <- sub.df.hvst %>% 
  spread(metric, score) %>% 
  left_join(condition.df, by = c("bas_loc_rm", "site_condition", "lg_reg_v2")) %>% 
  select(site_condition, metrics.vec.hvst) %>% 
